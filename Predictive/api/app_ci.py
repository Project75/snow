...
json_file = open('../model/2/model_ci.json', 'r')
loaded_model_json = json_file.read()
json_file.close()
keras_model_loaded = model_from_json(loaded_model_json)
keras_model_loaded.load_weights('../model/2/model_ci.h5')
print('Model loaded...')


#load tokenizer pickle file
with open('../model/2/tokenizer_ci.pickle', 'rb') as handle:
    tok = pickle.load(handle)
with open('../model/2/le_ci.pickle', 'rb') as handle2:
    le = pickle.load(handle2)

def preprocess_text(texts,max_review_length = 30):
...
@app.route('/servicenow/incidentci',methods=['POST'])
...
def predict():
    ...
    with graph.as_default():
        ...
        data['Configuration Item'] = y1
        #data['score'] = str(round(max(y[0, :])*100,2))+"%"
        return jsonify(data)
